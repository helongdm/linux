...
#define ADF4377_MAX_VCO_FREQ		(12800 * HZ_PER_MHZ)
#define ADF4377_MIN_VCO_FREQ		(6400 * HZ_PER_MHZ)
...
int adf4377_set_freq(struct adf4377_state *st)
{
	...
	return regmap_write(st->regmap, 0x10,  FIELD_PREP(ADF4377_N_INT_LSB_MSK, st->n_int));
}

static int adf4377_init(struct adf4377_state *st)
{
	u16 synth_lock_timeout, vco_alc_timeout, adc_clk_div, vco_band_div;
	u8 dclk_div1, dclk_div2, dclk_mode;
	...
	ret = regmap_update_bits(st->regmap, ADF4377_REG(0x11),
				 ...
				 FIELD_PREP(ADF4377_DCLK_DIV2_MSK, dclk_div2));
	if (ret)
		return ret;
	...
	ret = regmap_update_bits(st->regmap, ADF4377_REG(0x2F),
				 ...
				 FIELD_PREP(ADF4377_DCLK_DIV1_MSK, dclk_div1));
	if (ret)
		return ret;

	ret = regmap_update_bits(st->regmap, ADF4377_REG(0x24), ADF4377_DCLK_MODE_MSK,
				 FIELD_PREP(ADF4377_DCLK_MODE_MSK, dclk_mode));
	if (ret)
		return ret;

	ret = regmap_write(st->regmap, ADF4377_REG(0x27),
			   FIELD_PREP(ADF4377_SYNTH_LOCK_TO_LSB_MSK, synth_lock_timeout));
	if (ret)
		return ret;
	...
	ret = regmap_update_bits(st->regmap, ADF4377_REG(0x28),
				 ...
				 FIELD_PREP(ADF4377_SYNTH_LOCK_TO_MSB_MSK,
					    synth_lock_timeout >> 8));
	if (ret)
		return ret;

	ret = regmap_write(st->regmap, ADF4377_REG(0x29),
			   FIELD_PREP(ADF4377_VCO_ALC_TO_LSB_MSK, vco_alc_timeout));
	if (ret)
		return ret;

	ret = regmap_update_bits(st->regmap, ADF4377_REG(0x2A),
				 ADF4377_VCO_ALC_TO_MSB_MSK,
				 FIELD_PREP(ADF4377_VCO_ALC_TO_MSB_MSK, vco_alc_timeout >> 8));
	if (ret)
		return ret;

	ret = regmap_write(st->regmap, ADF4377_REG(0x26),
			   FIELD_PREP(ADF4377_VCO_BAND_DIV_MSK, vco_band_div));
	if (ret)
		return ret;

	ret = regmap_write(st->regmap, ADF4377_REG(0x2D),
			   FIELD_PREP(ADF4377_ADC_CLK_DIV_MSK, adc_clk_div));
	if (ret)
		return ret;
...
#define _ADF4377_EXT_INFO(_name, _ident) { \
		.name = _name, \
		.read = adf4377_read, \
		.write = adf4377_write, \
		.private = _ident, \
